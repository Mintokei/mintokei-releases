...
- **Source code is private** (`chovik/mintokei-new`). Only compiled, signed binaries and the update manifest are published here — a public binary does not expose private source.
...
- `source_ref` — the ref/sha in `chovik/mintokei-new` to build
...
| Secret | Purpose |
|---|---|
| `SOURCE_TOKEN` | read access to private `chovik/mintokei-new` |
