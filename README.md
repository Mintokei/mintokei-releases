...
- **Source code is private** (`Mintokei/mintokei-new`). Only compiled, signed binaries and the update manifest are published here — a public binary does not expose private source.
...
- `source_ref` — the ref/sha in `Mintokei/mintokei-new` to build
...
| Secret | Purpose |
|---|---|
| `SOURCE_TOKEN` | read access to private `Mintokei/mintokei-new` |
